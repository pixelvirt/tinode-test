...
const messages = defineMessages({
    ...
    cred_confirmed_successfully: {
        ...
        description:
            "Notification message that the credential was successfully validated.",
    },
    password_reset_success: {
        id: "password_reset_success",
        defaultMessage: "Password reset successfully",
        description:
            "Notification message that the password was successfully reset.",
    },
});
...
class TinodeWeb extends React.Component {
    constructor(props) {
        ...
        this.handleShowCountrySelector =
            this.handleShowCountrySelector.bind(this);
    ...
    componentDidMount() {
        ...
        this.setState({ ...this.state, saveToken: true });
    }
    ...
    initFCMessaging() {
        ...
        try {
            this.fcm = firebaseGetMessaging(
                firebaseInitApp(FIREBASE_INIT, APP_NAME)
            );
            ...
            Object.assign(
                ...
                TinodeWeb.stateForError(
                    this.props.intl.formatMessage(
                        messages.cred_confirmed_successfully
                    ),
                    "info"
                )
            );
    ...
    tnInitFind() {
        ...
        if (fnd.isSubscribed()) {
            ...
            fnd.subscribe(fnd.startMetaQuery().withSub().build()).catch(
                (err) => {
                    this.handleError(err.message, "err");
                }
            );
   ...
   */
    ...
    handleTopicSelected(topicName) {
        ...
        if (topicName) {
            ...
            if (this.state.topicSelected != topicName) {
                ...
                HashNavigation.navigateTo(
                    HashNavigation.setUrlTopic("", topicName)
                );
            }
        } else {
    ...
    handleSidepanelCancel() {
        ...
        let path = "";
        if (
            ["security", "support", "general", "notif"].includes(parsed.path[0])
        ) {
            path = "edit";
            ...
            if (pub) {
                if (pub.photo) {
                    if (pub.photo.ref && pub.photo.ref != Tinode.DEL_CHAR) {
                        ...
                        !pub.photo.data ||
                        pub.photo.data == Tinode.DEL_CHAR
                    ) {
    ...
    handleHideForwardDialog(keepForwardedMessage) {
        this.setState({
            forwardDialogVisible: false,
            forwardMessage: keepForwardedMessage
    ...
    handleInfoMessage(info) {
        ...
        switch (info.event) {
            case "accept":
                ...
                if (
                    Tinode.isMeTopicName(info.topic) &&
                    this.tinode.isMe(info.from)
                ) {
                    this.setState({
                        callTopic: null,
                        callState: CALL_STATE_NONE,
